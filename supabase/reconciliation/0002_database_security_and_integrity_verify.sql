...
-- 1. Función pública y validador interno: ambos deben existir.
select
...
-- 2. Trigger programado: debe devolver una fila habilitada.
...
where n.nspname = 'public'
  ...
  and t.tgname = 'validate_activities_scheduled_state'
...
-- 4. Ambas RPC de asistencia deben validar el plazo y usar el error acordado.
...
-- 5. No debe existir una fila scheduled incompatible.
select a.id
...
-- 6. EXECUTE: las tres consultas de desviaciones deben devolver cero filas.
...
-- authenticated conserva las 30 firmas baseline y publish_activity; el trigger
-- interno nuevo no requiere EXECUTE de cliente.
select p.oid::regprocedure as missing_authenticated_execute
from pg_proc p
join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname <> 'validate_activity_scheduled_state'
...
-- 7. anon: resultado esperado, una sola fila system_health/SELECT.
select table_name, privilege_type
from information_schema.table_privileges
where table_schema = 'public' and grantee = 'anon'
order by table_name, privilege_type;

-- 8. Contrato directo exacto de authenticated: debe devolver cero filas.
...
-- 9. Secuencia: las seis celdas deben ser false.
select role_name,
...
-- 10. A-02 diferido: debe ser true; no concede acceso, sólo confirma que 0002 no
