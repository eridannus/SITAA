...
El snapshot vigente posterior a 0006 fue generado en `2026-07-18T04:05:40Z`. La reconciliación confirmó 17 tablas, 156 columnas, 72 constraints, 38 índices, 8 triggers públicos, 39 firmas de función, 23 políticas y 51 filas de semillas. Los privilegios suman 112 grants de rutina, 261 de tabla, 6 de secuencia y 413 entradas ACL expandidas. No se encontraron inconsistencias entre el esquema principal y los snapshots especializados. Los índices de PK y UNIQUE se consideran representados por sus constraints aunque no aparezcan como sentencias `CREATE INDEX` independientes en el dump.

La comparación contra `0001`–`0006` no encontró deriva inexplicada. El informe `0006_post_apply_reconciliation.md` registra la comparación estructural, funcional, RLS, privilegios, catálogos y diferencias controladas de datos operativos. `live_triggers.sql` cubre tablas de `public`; los triggers de `auth.users` se verifican mediante los preflight y verificadores transaccionales aprobados. Las ACL por columna no forman parte de estos artefactos; el verificador 0006 comprobó el `UPDATE` acotado de nombres bajo el rol `authenticated`.
...
La cadena reconciliada actual sigue siendo `0001`–`0006`. `0007_admin_account_directory_audit.sql` ya se aplicó con `COMMIT`, su preflight fue aprobado y la aplicación compatible está publicada; la migración es inmutable. La primera ejecución del verificador falló antes de crear fixtures por un defecto de normalización del arnés, no por deriva de objetos o ACL persistentes. La reejecución corregida, los smoke tests, el snapshot posterior a 0007 y la reconciliación `0001`–`0007` permanecen pendientes. Los artefactos de `live/` siguen siendo evidencia post-0006 y no deben editarse para anticipar el cierre. No existe una migración 0008 para esta corrección del verificador.
