...
Los artefactos `0009_admin_account_lifecycle_transitions_{preflight,verify,rollback}.sql` están preparados para B.2b. El preflight sólo devuelve categorías/conteos y termina en `ROLLBACK`; el verificador usa fixtures sintéticas dentro de una transacción y termina en `ROLLBACK`; el rollback elimina únicamente las tres funciones 0009 y no revierte transiciones ni auditoría ya confirmadas. El verificador y el rollback no se han ejecutado contra Supabase; 0009 permanece no aplicada.
...
El primer intento de migración falló mientras PostgreSQL compilaba el `DO $preflight$` inicial: la rama de `is_b1_account_admin()` no cerraba su `EXISTS` exterior antes del siguiente `UNION ALL`. Sólo se ejecutaron `BEGIN` y los dos `SET LOCAL`; no se alcanzaron DDL ni `COMMIT`. Tras corregir la sintaxis y reforzar el balance de cuerpos dollar-quoted, el segundo intento entró al preflight y falló al calcular el hash pre-DDL de privilegios predeterminados porque `pg_default_acl.defaclobjtype` (`pg_catalog."char"`) se concatenaba sin `::text`. Tampoco alcanzó el primer `CREATE FUNCTION`, `REVOKE`, `GRANT`, la guarda post-DDL ni `COMMIT`. Ambas transacciones se descartaron, no crearon objetos 0009 y no requirieron rollback. Las dos serializaciones default ACL están corregidas localmente; 0009 permanece no aplicada, un tercer intento controlado está pendiente y B.2b continúa abierta.
