...
La cadena reconciliada actual es `0001`–`0010`. 0007 cerró Fase B.1, 0008 cerró Fase B.2a, 0009 cerró Fase B.2b y 0010 cerró Fase B.3a dentro de sus alcances aprobados. Los artefactos de `live/` son la evidencia autoritativa post‑0010 y no se editan manualmente. B.3b y Fase C permanecen pendientes. `0011` está asignada a `SEM-01`, aplicada en producción y aprobada por su verificador transaccional, pero todavía no ha sido reconciliada.

## Checkpoint aplicado y verificado de 0011 / SEM-01

Los archivos `0011_academic_period_administration_{preflight,verify,rollback}.sql`, la migración homónima, `scripts/check-sql-0011.mjs` y `docs/TEST_PLAN_0011.md` forman el paquete de base. La migración fue aplicada correctamente el 2026-08-12. El primer intento del verificador fue rechazado y no aporta casos aceptados; el segundo, con el verificador corregido, aprobó el conjunto exacto 1–51 y terminó con su propio `ROLLBACK` explícito. El rollback de la migración no fue ejecutado.

La evidencia sanitizada está en `0011_academic_period_administration_verification_evidence.md`. El siguiente gate es el arnés multisesión real; después deben regenerarse los artefactos de `live/` y reconciliarse post-0011. Mientras tanto, `live/` continúa describiendo exclusivamente post-0010 y no debe editarse ni interpretarse como inventario post-0011. `/admin/periods` y sus smoke tests permanecen pendientes, y SEM-01 no está cerrado.

El código fuente del gate multisesión ya está preparado en `scripts/sem01_0011_run_multisession.ps1` y su contrato local se verifica mediante `scripts/check-sem01-0011-multisession.mjs`. Ambos están validados sólo de forma local: el probe LAB y la matriz MS01–MS24 no se han ejecutado ni han establecido conexión con una base de datos. Sus artefactos operativos deben permanecer fuera del repositorio, bajo `C:\Dev\SITAA-LOCAL-EVIDENCE\SEM-01\MULTISESSION-0011`, hasta que una ejecución aprobada produzca un checkpoint sanitizado. No se puede regenerar el snapshot post-0011 antes de aprobar esa matriz.
