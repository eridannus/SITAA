...
La primera ejecución del verificador post-aplicación aprobó sus controles estáticos y comenzó las fixtures, pero abortó porque el arnés llamó directamente bajo `authenticated` a `is_b1_account_admin()`, helper privado owner-only. La denegación `42501` fue correcta y la transacción completa se descartó sin persistencia. La corrección local separa la semántica bajo owner, la denegación ACL cliente y el acceso interno desde las RPC B.1/B.2a `SECURITY DEFINER`. La reejecución del verificador, los smoke tests, la regeneración del snapshot y la reconciliación `0001`–`0008` están pendientes.
