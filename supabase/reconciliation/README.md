...
El primer intento de migración falló mientras PostgreSQL compilaba el `DO $preflight$` inicial: la rama de `is_b1_account_admin()` no cerraba su `EXISTS` exterior antes del siguiente `UNION ALL`. Sólo se ejecutaron `BEGIN` y los dos `SET LOCAL`; no se alcanzaron DDL, grants, guarda post-DDL o `COMMIT`, no se creó objeto 0009 alguno y el rollback no fue necesario. El arnés está corregido localmente y ahora valida cada cuerpo dollar-quoted por separado, pero la migración corregida aún no se ha reejecutado. B.2b continúa abierta.
