...
La cadena reconciliada actual es `0001`–`0010`. 0007 cerró Fase B.1, 0008 cerró Fase B.2a, 0009 cerró Fase B.2b y 0010 cerró Fase B.3a dentro de sus alcances aprobados. Los artefactos de `live/` son la evidencia autoritativa post‑0010 y no se editan manualmente. B.3b y Fase C permanecen pendientes. `0011` está asignada a `SEM-01` y preparada localmente, pero todavía no ha sido aplicada ni reconciliada.

## Paquete local 0011 de SEM-01

Los archivos `0011_academic_period_administration_{preflight,verify,rollback}.sql`, la migración homónima, `scripts/check-sql-0011.mjs` y `docs/TEST_PLAN_0011.md` forman un paquete local pendiente de revisión. El preflight no se ha ejecutado remotamente; tampoco se han ejecutado migración, verificador o rollback. Los artefactos de `live/` continúan describiendo exclusivamente post-0010 y no deben alterarse hasta completar el flujo manual aprobado.
