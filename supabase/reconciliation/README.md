...
La primera ejecución del verificador post-aplicación abortó porque el arnés llamó directamente bajo `authenticated` a `is_b1_account_admin()`, helper privado owner-only; la denegación `42501` fue correcta. La segunda aprobó esa corrección, los contratos previos, fixtures, rechazos y siete mutaciones RPC, pero abortó porque las postcondiciones crudas de perfiles, auditoría e historia continuaban dentro del intervalo `authenticated`. Ambas transacciones se descartaron sin persistencia. La segunda corrección local conserva las pruebas públicas/DML en rol cliente, valida allí la historia B.1 sanitizada y restablece owner para estado y auditoría crudos, con la frontera histórica dividida en owner/cliente/owner. La reejecución del verificador, los smoke tests, la regeneración del snapshot y la reconciliación `0001`–`0008` están pendientes.

El inventario contractual post-0008 es 18/165/80/43/11/51/25/51 para tablas/columnas/restricciones/índices/triggers/funciones/políticas/semillas. A partir de la representación post-0007, los privilegios esperados son 132 grants de rutina, 267 de tabla, 6 de secuencia y 440 ACL expandidas. `live/` continúa representando post-0007; estos totales no son evidencia viva hasta regenerar el snapshot. 0009 es el siguiente número disponible, pero la corrección exclusiva del verificador no requiere crearlo.
