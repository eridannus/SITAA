...
La primera ejecución remota del preflight 0008 terminó con `ROLLBACK` y mostró un único bloqueo `registration_trigger_drift = 1`; fue un falso positivo porque el artefacto local buscaba nombres no canónicos. La revisión local ahora valida exactamente `on_sitaa_auth_user_created` y `on_sitaa_auth_user_email_changed` mediante `pg_trigger`, atributos de evento/columna/condición y los OID de sus handlers. La migración sólo preserva y verifica esos triggers; no contiene DDL sobre ellos. El resultado temporal de aquella ejecución no forma parte de los artefactos de reconciliación y la reejecución corregida está pendiente.
