...
El snapshot vigente posterior a 0005 fue generado en `2026-07-17T23:20:07Z`. La reconciliación confirmó 17 tablas, 156 columnas, 68 constraints, 38 índices, 7 triggers públicos, 37 firmas de función, 23 políticas y 51 filas de semillas. Los privilegios suman 108 grants de rutina, 261 de tabla, 6 de secuencia y 409 entradas ACL expandidas. No se encontraron inconsistencias entre el esquema principal y los snapshots especializados. Los índices de PK y UNIQUE se consideran representados por sus constraints aunque no aparezcan como sentencias `CREATE INDEX` independientes en el dump.

La comparación contra `0001`–`0005` no encontró deriva inexplicada. El informe `0005_post_apply_reconciliation.md` registra la comparación estructural, funcional, RLS, privilegios, catálogos y diferencias controladas de datos operativos. `live_triggers.sql` cubre tablas de `public`; los triggers de `auth.users` se verifican mediante los preflight y verificadores transaccionales aprobados.
