...
## Aplicación 0010 y verificación pendiente
...
La aplicación compatible fue desplegada correctamente, la Edge Function está `ACTIVE` y 0010 fue aplicada; el registro local termina en `COMMIT`. La Edge no se invocó, no hubo Auth Admin ni una operación real B.3a. El primer verificador hospedado terminó con código 3 en `restore_failure_finalize`: la función emitió el contrato correcto `42501/sitaa_account_lifecycle_auth_unconfirmed`, mientras el arnés esperaba `P0001`. No alcanzó el `ROLLBACK` final y la desconexión de `psql` descartó la transacción abierta.

El handler corregido usa `insufficient_privilege` y exige el SQLSTATE y mensaje exactos. El snapshot autoritativo continúa siendo post‑0009 y no debe incorporar 0010 hasta aprobar la reejecución completa, la prueba Auth desechable, los smoke tests y una nueva reconciliación. Las semánticas hospedadas de `ban_duration`/sesión siguen sin verificar, B.3a continúa abierta y no se debe crear 0011.
