...
La primera ejecución remota del preflight 0008 terminó con un falso positivo por nombres; la segunda abortó antes de las categorías porque `pg_get_expr` no decompila el `WHEN` con `OLD` y `NEW`. La tercera devolvió las 40 categorías y terminó con `ROLLBACK`; todos los bloqueos fueron cero salvo `registration_trigger_drift = 1`. Un diagnóstico canónico de sólo lectura confirmó ambos nombres, pares handler/trigger y cero extras. El falso positivo restante fue sólo `::text` en la salida de `pg_get_triggerdef`. La revisión local aísla ahora el `WHEN`, elimina exclusivamente ese cast y compara por igualdad exacta. La migración no contiene DDL sobre esos triggers. Las salidas temporales no forman parte de la reconciliación y otra reejecución permanece pendiente.
