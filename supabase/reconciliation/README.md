...
Los artefactos `0009_admin_account_lifecycle_transitions_{preflight,verify,rollback}.sql` están preparados para B.2b. El preflight sólo devuelve categorías/conteos y termina en `ROLLBACK`; el verificador usa fixtures sintéticas dentro de una transacción y termina en `ROLLBACK`; el rollback elimina únicamente las tres funciones 0009 y no revierte transiciones ni auditoría ya confirmadas. La migración, el verificador y el rollback no se han ejecutado contra Supabase.

El primer preflight remoto 0009 sí se ejecutó posteriormente como transacción de sólo lectura: devolvió 26 filas, terminó con `ROLLBACK` y no fue aprobado por cuatro falsos positivos. Un diagnóstico separado, también de sólo lectura y terminado con `ROLLBACK`, confirmó que los conteos y contratos post-0008 seguían intactos. Los artefactos locales se corrigieron para usar `pg_get_constraintdef(..., true)` en mapas de restricciones, `pg_get_triggerdef(..., true)` sólo en el mapa agregado de triggers públicos, la expansión exacta de seis ACL de secuencia y el mapa de diecinueve grants de tabla `authenticated` posterior a 0008. El parser especializado del `WHEN` Auth conserva `pg_get_triggerdef(..., false)`. La versión corregida del preflight aún no se ha reejecutado; 0009 permanece no aplicada y B.2b no está cerrada.
