...
### Evidencia del preflight y de los intentos de migración
...
109. Los dos hashes de `pg_default_acl` serializan `defaclobjtype::text` con la misma representación, separador, tratamiento nulo y orden; el checker rechaza la concatenación directa del tipo interno `"char"`.
110. La captura de la línea base default ACL permanece antes del primer DDL persistente y su comparación permanece dentro de la guarda post-DDL antes de `COMMIT`.
