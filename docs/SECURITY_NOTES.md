...
- La aplicación no necesita DML directo sobre `activity_participants`: 0008 conserva sólo `SELECT` para `authenticated`, elimina su DML de tabla y exige ausencia total de ACL de columna, grantees inesperados y grant option. Preflight, post-DDL, verificador y rollback revisan `attacl`, `aclexplode`, `column_privileges` y privilegios efectivos. Las escrituras directas de `activities` se conservan para el flujo actual, pero un trigger impide sustituir creador/responsable, revalida cambios de alcance y bloquea cambios que dejarían participantes incompatibles.
