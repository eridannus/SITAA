...
- `/acerca-de` y `/privacidad` son rutas públicas estáticas, sin consulta de sesión, analytics, tracking, cookies adicionales ni secretos de infraestructura. El lanzamiento inicial no requiere una ruta de términos.
...
- La aplicación bloquea perfiles pendientes o inactivos. La desactivación/reactivación operativa auditada está implementada por 0009; 0010 implementa en B.3a la suspensión/restauración coordinada para esas transiciones de ciclo de vida, mientras las demás operaciones Auth permanecen en B.3b.
...
- La desactivación B.2b activa inmediatamente la barrera operativa 0008 aun cuando un JWT siga vigente, pero por sí sola no elimina ni modifica Auth, identidad, asignaciones, actividades, responsabilidades, participaciones, asistencia o historia. La reactivación no repara asignaciones vencidas, futuras, inactivas, malformadas o incompatibles. B.3a implementa mediante 0010 la suspensión/restauración Auth coordinada para estas transiciones; las demás operaciones `auth.admin` permanecen en B.3b y Fase C conserva la mutación de roles.

### B.3a: límite Auth confiable desplegado, probado y reconciliado
...
- La suspensión no se describe como revocación criptográfica universal de JWT. No se usa `auth.admin.signOut()` sin un JWT de la sesión objetivo y nunca se sustituye por el token del administrador.
- La matriz central en un proyecto desechable observó que la desactivación coordinó el perfil inactivo y el ban Auth: dos JWT existentes, dos refresh tokens y dos logins nuevos fueron rechazados con `user_banned`, mientras las operaciones protegidas de SITAA intentadas con los dos tokens quedaron denegadas (`2/2`).
- La restauración con `ban_duration = 'none'` eliminó el ban y permitió un login fresco, pero no reactivó los refresh tokens anteriores. Una cuenta reactivada debe iniciar una sesión nueva; no se debe prometer recuperación automática de pestañas o sesiones previas.
- `user_banned` y la no recuperación de refresh tokens son evidencia empírica de la ejecución hospedada del 3 de agosto de 2026, no una garantía permanente para futuras versiones de Supabase.
...
- La matriz failure/recovery v11 confirmó en una ejecución desechable concreta que un fallo Auth controlado reintentable no duplica el evento de ciclo. Una vez alcanzada `auth_synchronized`, la recuperación finaliza sin repetir la llamada Auth.
- Otra autoridad B.1 exacta puede completar una operación varada: Admin A permaneció como solicitante y Admin B quedó como finalizador. La evidencia conserva esa separación sin publicar sus identificadores.
...
- La idempotencia de `request_id` usa una sola restricción e índice `admin_auth_operations_request_id_key`; el catálogo debe demostrar por `conindid` que son el mismo objeto físico. La preparación rechaza explícitamente transición `NULL`, vacía, desconocida o con mayúsculas antes de cualquier cambio. Los guards embebido y de rollback comparan el baseline canónico post‑0009 completo, no sólo conteos o hashes capturados de un estado desconocido.
...
- El resultado privilegiado incluye el `attempt_count` exacto reclamado. La base lo compara después del row lock y rechaza un resultado tardío con `sitaa_auth_operation_stale_attempt` antes de mutar ledger, timestamps o auditoría.
- Los timestamps autoritativos de prepare, claim, record y finalize usan un único `clock_timestamp()` capturado después de los locks y nunca retroceden respecto de `updated_at`. La vista de contexto usa `statement_timestamp()` sólo como indicación; claim decide el lease.
- Una evidencia ya establecida no puede reemplazarse: `profile_audit_event_id`, `auth_audit_event_id`, `auth_synchronized_at` y `completed_at` permanecen idénticos. `terminal_failure` sólo puede registrarse antes de `auth_synchronized` y sin un éxito Auth previo.
- La clasificación de fallos Auth continúa provisional y reintentable para 400/401/403/404/422, límites, red y errores desconocidos. En especial, un error de credencial/configuración o un Auth user inesperadamente ausente no convierte una cuenta inactiva en un callejón terminal sin recuperación. La matriz failure/recovery sólo inyectó de forma controlada `auth_temporarily_unavailable`; no demuestra que todo error real o futuro del proveedor sea reintentable.
- El verificador 0010 no usa privilegios cliente para observar producción: `authenticated` y `service_role` sólo invocan sus RPC aprobadas, ejercitan dos denegaciones ACL exactas o escriben resultados sanitizados en tablas `pg_temp` concedidas expresamente. Después de cada `RESET ROLE`, el owner comprueba perfiles, Auth, asignaciones, ledger, auditoría, timestamps, hashes y evidencia; las regresiones de escritura directa del trigger también son owner-only. El checker estático rechaza cualquier referencia cruda protegida adicional dentro de los intervalos cliente.
- La guarda predestructiva del rollback separa el ACL de 135 entradas preexistentes, la única entrada owner-only del mutador B.2b y la matriz exacta de las seis funciones B.3a. El ACL completo post‑0009 de 137 entradas sólo se exige después de retirar 0010 y restaurar el grant histórico.
- `FunctionsHttpError` no equivale automáticamente a indisponibilidad: su cuerpo JSON se lee una vez y se valida con el parser exacto para conservar resultados de negocio sanitizados 403/409. Relay, red, errores desconocidos o cuerpos malformados fallan cerrados; nunca se analiza texto crudo ni se activa el fallback 0009 por un error Edge.
- Claim, record y finalize vuelven a comprobar la autoridad B.1 exacta después del advisory lock y del `FOR UPDATE`. Una revocación mientras el actor espera impide replay y mutación; `42501` se conserva en Edge como `authorization_lost/pending` con el UUID conocido y nunca activa el fallback 0009.
- El parser Edge acepta únicamente combinaciones discriminadas de código, estado y `operationId`; rechaza llaves adicionales, estados/códigos desconocidos, UUID ausente y cruces de transición. Los campos internos `char` de catálogos PostgreSQL se convierten a texto explícitamente antes de concatenarlos o hashearlos.
- El primer preflight remoto 0010 fue de sólo lectura, devolvió 34 categorías y terminó con `ROLLBACK`/código 0 sin cambios. No fue aprobado porque `dangerous_default_acl` contó 50 defaults estándar: 29 de 30 bloqueos sí fueron cero. El diagnóstico de sólo lectura confirmó cinco grupos de diez filas (`postgres/public`, `postgres/storage`, `supabase_admin/graphql`, `supabase_admin/graphql_public` y `supabase_admin/public`) y también terminó con `ROLLBACK`/código 0 sin cambiar privilegios predeterminados.
- La corrección de `dangerous_default_acl` falla cerrada si `current_user` o `session_user` no son `postgres`; sólo considera defaults creados por `postgres`, globales o de `public`, para tablas y funciones, y bloquea cualquier grantee que no sea `PUBLIC`, `anon`, `authenticated`, `service_role` o el owner. Los defaults de otros propietarios, otros esquemas y secuencias no inicializan los objetos 0010.
- La segunda ejecución corregida quedó aprobada: 34 filas, sus 30 bloqueos en cero, `dangerous_default_acl = 0` y cuatro conteos informativos 1/4/0/0. Finalizó con `ROLLBACK`, código 0 y sin `ERROR`; no expuso datos sensibles ni cambió objetos, filas o privilegios.
- 0010 no modifica privilegios predeterminados de la plataforma. Después de crear el ledger revoca roles conocidos, retira dinámicamente cualquier otro grantee no owner, restaura el ACL completo del owner y exige ocho entradas owner-only, cero ACL de columna, RLS y cero políticas. Las seis funciones conservan revocaciones explícitas y mapas bidireccionales exactos. El hash completo de `pg_default_acl` se captura antes del DDL persistente y debe permanecer idéntico antes de `COMMIT`.
- La aplicación compatible fue desplegada, la Edge Function está `ACTIVE` y 0010 fue aplicada con `COMMIT`. La matriz central ejecutó dos operaciones B.3a reales en un proyecto desechable y produjo cuatro eventos esperados, sin fallos Auth ni errores crudos persistidos.
- La matriz failure/recovery v11 aprobó los casos 13–15 en una única ejecución desechable: fallo Auth controlado y reintento idempotente, fallo de finalización posterior a `auth_synchronized` sin repetir Auth y recuperación por una segunda autoridad B.1 exacta. El postcheck registró cuatro operaciones completadas, ocho eventos administrativos esperados, cero operaciones no exitosas y cero eventos Auth de fallo.
- El primer verificador hospedado abortó en una prueba negativa esperada: `sitaa_account_lifecycle_auth_unconfirmed` usa `42501`, pero el arnés lo capturaba como `P0001`. La desconexión descartó la transacción que no alcanzó su `ROLLBACK` final. El diagnóstico de sólo lectura posterior confirmó ledger existente, seis funciones B.3a, cero operaciones y cero eventos Auth B.3a; terminó con `ROLLBACK` y código 0.
- La reejecución corregida capturó `insufficient_privilege`, exigió SQLSTATE y mensaje exactos, completó todos los escenarios y terminó con exactamente un `ROLLBACK`, código 0 y sin `ERROR`. El gate PostgreSQL está aprobado y no persistió fixtures, privilegios temporales, operaciones o auditoría.
- Las evidencias central y failure/recovery, sus postchecks, la salida y la auditoría fueron revisados sin correos, UUID, tokens, claves, contraseñas, URI, cookies, cabeceras `Authorization`, metadata sensible ni errores crudos. Los logs y checkpoints conservan sólo códigos estables y conteos sanitizados. Estos resultados corresponden a ejecuciones desechables concretas.
- La matriz de concurrencia y límites v8 separó dos pruebas del caso 17: las fixtures ordinarias de profesor y alumno quedaron denegadas en las superficies B.3a durante la fase PostgreSQL transaccional, sin mutar ledger ni auditoría; por otra parte, Target C autenticado sin autoridad B.1 fue rechazado en `start` y `retry` por la ruta Hosted. Los hashes de perfil, ledger y auditoría permanecieron iguales y el postcheck confirmó la identidad Auth del objetivo. Para el caso 18, `service_role` no leyó ni mutó directamente `admin_auth_operations`; dentro de B.3a conservó únicamente claim/result, sin modificar su ACL histórico `SELECT`/`INSERT` sobre `admin_audit_events` ni concederle `UPDATE`, `DELETE` o `TRUNCATE`.
- La espera real por advisory lock fue observada. Claim, record y replay final revalidaron autoridad después de esperar y fallaron cerrados cuando el actor perdió B.1; no devolvieron replay ni mutaron ledger, intento, timestamps o auditoría.
- Un lease fresco no se recuperó prematuramente, la recuperación posterior a cinco minutos fue aprobada y el cercado rechazó el intento obsoleto antes de aceptar el vigente. Otra autoridad B.1 exacta recuperó la operación `auth_synchronized` sin repetir Auth: una llamada durante la recuperación y cero repeticiones.
- La evidencia v8, su postcheck y el checkpoint versionado permanecen sanitizados. La normalización previa de cuatro campos de texto perteneció únicamente a una Authority D sintética desechable y no constituye migración, reparación de producción ni recomendación general sobre Auth administrado por Supabase.
- La auditoría productiva del caso 19 revisó los nombres y alcances de las tres variables públicas del proyecto en Vercel, confirmó cero variables compartidas y cero nombres visibles de credenciales privilegiadas o de base de datos; ningún valor fue revelado o copiado. Los logs Production observados registraron 7 respuestas HTTP 200 y cero warnings, errores, fatales, mensajes o hallazgos sensibles visibles.
- La revisión completa aprobó 22 artefactos locales de navegador, 532 artefactos de servidor, 14 respuestas de rutas productivas, 12 recursos estáticos remotos y 11 recursos JavaScript remotos. No encontró JWT privilegiado, secreto prohibido ni cliente privilegiado de primera parte. Las 280 apariciones de `service_role` y 8 de `SUPABASE_SECRET_KEY` en source maps de servidor eran literales textuales de dependencias sin valor asociado ni presencia en primera parte, navegador o recursos remotos; no son credenciales filtradas.
- El smoke test productivo controlado del caso 20 confirmó que el usuario ordinario no veía controles administrativos. La desactivación coordinó perfil inactivo y ban Auth; la sesión existente fue denegada al refrescar y un login fresco no creó sesión. La interfaz mostró mensajes genéricos y no expuso el error crudo del proveedor.
- La reactivación retiró el ban antes de confirmar el perfil activo. Una sesión nueva accedió correctamente y regresó a `/activities`; asignación, actividades, asistencia, identidad e historia permanecieron conservadas. Las dos operaciones finales y cuatro eventos append-only quedaron completos y sin error estable.
- Esta observación corresponde a un único recorrido productivo controlado con una cuenta ficticia y no constituye una garantía universal de la semántica presente o futura de Supabase. La telemetría directa del dashboard Edge estaba diferida y no se afirma un conteo de invocaciones observado.
- La primera operación real revocó definitivamente la elegibilidad del rollback 0010. La barrera SITAA permanece como defensa independiente aunque cambie la semántica futura del proveedor.
...
- **Estados técnicos de routing:** la alerta usa `registered`, `routing_pending`, `routed` y `routing_failed`. La interacción del destinatario se registra separadamente mediante los conceptos `delivered_at`, `read_at` y `archived_at`.
- **Inbox autoritativo:** el inbox interno y sus marcas de entrega/interacción son la fuente de verdad técnica. `read_at` sólo indica apertura y `archived_at` sólo organización o retiro del inbox activo; ninguna marca demuestra atención al estudiante, activación de un protocolo, investigación, resolución, descarte o seguimiento. Si se añade correo posteriormente, sólo incluirá un aviso genérico de que existe una alerta nueva; no contendrá alumno, materia, categoría ni contenido académico.
- **Sin resultado institucional:** SITAA sólo registra la alerta y el routing técnico realizado por el sistema. No almacena el resultado de la situación subyacente.
