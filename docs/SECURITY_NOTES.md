...
### B.3a: límite Auth confiable desplegado; verificador PostgreSQL aprobado
...
- La aplicación compatible fue desplegada, la Edge Function está `ACTIVE` sin invocación y 0010 fue aplicada con `COMMIT`. No se ejecutó Auth Admin ni una operación real B.3a.
...
- B.3a permanece abierta: no hubo invocación Edge, suspensión/restauración real ni Auth Admin. Siguen sin verificar `ban_duration`, JWT existentes, refresh tokens y la restauración con `ban_duration = 'none'`; faltan la matriz Auth desechable, smoke tests y reconciliación post‑0010.
