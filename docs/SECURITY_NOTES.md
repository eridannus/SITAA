...
### B.3a: límite Auth confiable desplegado; verificación pendiente
...
- El primer verificador hospedado abortó en una prueba negativa esperada: `sitaa_account_lifecycle_auth_unconfirmed` usa `42501`, pero el arnés lo capturaba como `P0001`. La desconexión descartó la transacción que no alcanzó su `ROLLBACK` final. La corrección captura `insufficient_privilege` y exige SQLSTATE y mensaje exactos; B.3a permanece abierta hasta aprobar la reejecución, matriz Auth, smoke tests y reconciliación post‑0010.
