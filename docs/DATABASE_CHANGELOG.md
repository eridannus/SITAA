...
- Corrección previa a aplicación: preflight independiente, preflight embebido, guarda post-DDL y verificador ahora exigen por catálogo los triggers canónicos `on_sitaa_auth_user_created` y `on_sitaa_auth_user_email_changed`, incluidas unicidad, relación, evento, timing, columnas, predicado, habilitación y función por OID. 0008 no modifica esos triggers; la reejecución remota permanece pendiente.
- 0001–0007 y el snapshot vivo permanecen inmutables. 0008 continúa local, no aplicada, no verificada en PostgreSQL, sin smoke tests y no reconciliada. No se ejecutó ni aplicó SQL durante esta preparación.
