...
- La restauración usa el valor tipado `ban_duration = 'none'`, pero no se ha comprobado contra Supabase hospedado. Sólo se ejecutaron el primer preflight y su diagnóstico de sólo lectura; no se aplicó la migración ni se ejecutaron verificador, Edge Function, Auth Admin o prueba desechable.
...
- Corrección local pendiente de reejecución: `dangerous_default_acl` sólo considera defaults de `postgres`, globales o de `public`, para tablas/funciones, y bloquea grantees fuera de `PUBLIC`, `anon`, `authenticated`, `service_role` y el owner. No se ejecutó `ALTER DEFAULT PRIVILEGES`; la normalización ACL de tabla/funciones y el hash completo de `pg_default_acl` permanecen obligatorios.
