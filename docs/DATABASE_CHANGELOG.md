...
- Cierre final de autorización: el preflight bloquea `attacl`, grants de columna o privilegios efectivos inesperados sobre `activity_participants`; el estado post-DDL exige owner y `service_role` completos, `authenticated` sólo `SELECT` de tabla y cero ACL de columna. El verificador incorpora una regresión temporal de grant por columna y el rollback comprueba el contrato antes de destruir y después de restaurar.
