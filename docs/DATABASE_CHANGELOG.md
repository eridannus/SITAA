...
- La restauración usa el valor tipado `ban_duration = 'none'`, pero no se ha comprobado contra Supabase hospedado. Se ejecutaron dos preflight de sólo lectura y el diagnóstico acotado; no se aplicó la migración ni se ejecutaron verificador, Edge Function, Auth Admin o prueba desechable.
...
- Estado: preparación B.3a abierta; 0010 continúa sin aplicar, el verificador no se ha ejecutado, la Edge Function continúa sin desplegar ni invocar y no hubo Auth Admin. El siguiente paso autorizado es publicar y confirmar el despliegue del commit compatible, desplegar la Edge Function revisada y sólo después aplicar 0010 y ejecutar su verificador. No crear 0011 ni describir B.3a como aplicada o cerrada.
