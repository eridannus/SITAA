...
**Decisión:** preparar 0007 con cuatro RPC `SECURITY DEFINER` de sólo lectura y una bitácora `admin_audit_events` append-only. La autoridad exacta exige cuenta activa y asignación actual `technical_admin/system/technical`, sin programa/división; `starts_at` y `ends_at` son fechas calendario inclusivas de Ciudad de México. El directorio no navega el padrón sin criterios, escapa comodines literales, limita página a 1–1 000 000 y tamaño a 1–50, exige coincidencia de rol/servicio/alcance en una misma asignación y recupera el total mediante la misma RPC si una página queda fuera de rango. La lista enmascara identificadores; el detalle expone sólo identidad necesaria y un booleano de confirmación por Auth o Google verificado. La auditoría bloquea `UPDATE`/`DELETE`/`TRUNCATE`, restringe metadata normalizada, concede a `service_role` sólo `SELECT`/`INSERT` y omite metadata en su proyección B.1.
