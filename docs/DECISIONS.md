...
**Endurecimiento de revisión:** la revalidación adquiere `SHARE` en orden fijo sobre `role_assignments`, `activities` y `activity_participants` antes del perfil objetivo. Las entradas colapsan whitespace, recortan y convierten vacío en `NULL`; el tipo institucional nulo y todo nombre derivado menor de dos caracteres fallan con contratos controlados. Preflight, guarda post-DDL, verificador y rollback exigen RLS/FK/ACL/firmas exactos. El verificador crea su propio semestre futuro después del último rango conocido y el rollback compara los 29 cuerpos en el dominio normalizado de `pg_proc.prosrc`.
