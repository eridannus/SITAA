...
Los eventos de ciclo B.2b y sincronización Auth son evidencias diferentes, no duplicación: el primero acredita el perfil; el segundo acredita la fase privilegiada con metadata allowlisted. No se almacenan errores crudos, tokens, correo ni identidad del proveedor. El valor de restauración `ban_duration = 'none'` y la duración larga de suspensión se aislaron conforme a los tipos instalados de `@supabase/auth-js`; su comportamiento en el servicio hospedado todavía requiere prueba empírica desechable. No se afirma invalidación inmediata de JWT ni de refresh tokens.
