...
**Decisión:** aplicar 0009 con contexto sin PII y una única RPC de transición auditada. Se prohíbe actuar sobre la cuenta propia, sobre registros pendientes o sobre la última autoridad B.1 exacta. La guarda de conteo de última autoridad se conserva como defensa en profundidad: bajo el contrato válido, un actor distinto con autoridad implica al menos dos administradores exactos activos, mientras el caso unitario es necesariamente propio y se rechaza antes. La mutación serializa la decisión, reautoriza al actor después de los locks y, al reactivar una cuenta institucional, bloquea con `FOR SHARE` el programa referenciado antes de validar y actualizar. El programa debe existir y continuar activo; una escritura concurrente se resuelve por espera y reevaluación, no por una lectura optimista. Las dependencias operativas se muestran como advertencias y se conservan. Las asignaciones no se reescriben: al reactivar, sólo recuperan efecto las que además continúen activas, vigentes por la fecha inclusiva de Ciudad de México y estructuralmente válidas; las vencidas, futuras, inactivas o malformadas no se reparan ni activan automáticamente.

**Consecuencias:** desactivar suspende el acceso por la barrera operativa existente aun si un JWT sigue técnicamente vigente, pero conserva identidad e historia y no revoca físicamente sesiones Supabase ni administra Auth. El preflight presenta 19 bloqueos y 7 informativos, exige el conjunto canónico de 51 semillas y preserva exactamente los triggers Auth y los tres grants de actualización de nombres estructurados; ninguna columna de ciclo de vida queda editable por cliente. El verificador separa fases cliente y owner, calcula la seguridad de dos administradores respecto de la línea base viva y conserva sin mutar a toda autoridad preexistente. Las operaciones `auth.admin`, recuperación y revocación física pertenecen a B.3; los roles V2 pertenecen a Fase C. Cada éxito genera evidencia administrativa append-only. El tercer intento de 0009 terminó con `COMMIT`, el verificador final con `ROLLBACK`, los smoke tests aprobaron y el snapshot `2026-07-22T23:32:46Z` fue reconciliado sin deriva inexplicada. 0009 es inmutable y B.2b está cerrada; la matriz manual multisesión permanece sin ejecutar en producción. A-02 continúa diferida intencionalmente hasta diseñar la administración de usuarios, roles y permisos.

**Estado:** Implementada, aplicada, verificada, reconciliada y cerrada mediante 0009.

## DEC-052 — Límite Auth confiable y coordinación B.3a

**Contexto:** B.2b cambia el estado operativo y la barrera 0008 niega operaciones de una cuenta inactiva, pero 0009 no administra Supabase Auth. PostgreSQL y Auth son sistemas distintos: no existe una transacción atómica que abarque ambos y una llamada privilegiada puede fallar después de confirmar una fase local.

**Decisión:** preparar 0010 con un ledger idempotente `admin_auth_operations`, etapas persistidas, reintentos y errores estables. La única superficie autorizada para `service_role` y `auth.admin.updateUserById()` es la Edge Function autenticada `admin-account-auth-lifecycle`; Next.js y el navegador usan sólo el JWT del usuario y nunca contienen el secreto. Desactivar confirma primero el perfil inactivo y después sincroniza Auth; reactivar restaura primero Auth y sólo activa el perfil mediante un finalizador autenticado. La barrera 0008 permanece como defensa inmediata. 0010 retira a `authenticated` la ejecución directa de la mutación B.2b y conserva un fallback de aplicación exclusivamente mientras la base reporte de forma explícita que B.3a no existe.

El hardening previo a aplicación fija una máquina de estados exacta, writers de columna allowlistados y marcador GUC nulo-seguro que se limpia después de cada DML. El advisory lock precede a la consulta autoritativa de `request_id`; un replay idéntico devuelve la operación existente en cualquier estado y uno incompatible falla con contrato estable. `current_operation_id` significa siempre la operación más reciente, incluso final. Un lease `processing` fresco sólo bloquea cuando Auth podría seguir ejecutándose; `processing/auth_synchronized` se recupera de inmediato para finalizar sin repetir Auth.
...
El verificador 0010 separa estrictamente ejecución cliente y observación owner: `authenticated` y `service_role` sólo invocan sus RPC aprobadas, realizan las denegaciones ACL exactas o escriben resultados sanitizados en `pg_temp`; toda postcondición cruda y toda mutación directa del trigger del ledger ocurre después de `RESET ROLE`. El rollback usa un mapa ACL dividido: 135 entradas preexistentes sin el mutador 0009 ni las seis funciones 0010, el mutador 0009 con una sola entrada owner-only y la matriz exacta de 0010; el mapa completo post‑0009 se exige sólo después de restaurar. Los cuerpos estables 403/409 de la Edge Function son resultados de negocio y se validan con el mismo parser exacto que HTTP 200; sólo los errores de transporte o cuerpos inválidos se convierten en `trusted_boundary_unavailable`. Ningún error Edge habilita el fallback 0009.
