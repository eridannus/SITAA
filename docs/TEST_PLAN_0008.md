...
La corrección usa `pg_get_triggerdef(trigger_oid, false)`, normaliza minúsculas, whitespace y paréntesis, y exige el fragmento contiguo exacto `whenold.emailisdistinctfromnew.emailexecutefunction`. Así no admite operandos invertidos, otra columna ni términos `AND`/`OR` adicionales. Una nueva reejecución remota del preflight corregido permanece pendiente.
...
Su contrato estático comprueba por separado nombre, unicidad, relación, condición no interna, habilitación ordinaria, función por OID, evento/timing, granularidad por fila, ausencia de columnas/`WHEN` en el alta, lista exacta `email` y predicado semántico en la sincronización. Para el `WHEN` con `OLD` y `NEW` usa `pg_get_triggerdef`, no `pg_get_expr`. También rechaza triggers adicionales que invoquen cualquiera de los dos handlers. No usa una sola existencia agrupada mediante `IN (...)` para aceptar ambos objetos.
