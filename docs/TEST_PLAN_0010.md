...
1. generar el snapshot canónico post‑0010;
2. reconciliarlo contra la cadena aplicada;
3. cerrar B.3a sólo cuando ese gate sea aprobado.
...
| # | Prueba | Estado | Evidencia requerida o comprobada |
| ---: | --- | --- | --- |
| 1 | Login base | Aprobada por matriz central | Login del objetivo sintético exitoso antes de suspender. |
| 2 | Refresh base | Aprobada por matriz central | Ambas sesiones renovaron antes de suspender. |
| 3 | JWT existente | Aprobada por matriz central | Se registró el resultado de cada JWT emitido sin convertirlo en garantía futura. |
| 4 | Refresh suspendido | Aprobada por matriz central | Se registró el comportamiento de refresh en ambas sesiones tras suspender. |
| 5 | Login nuevo suspendido | Aprobada por matriz central | Se registró el resultado de un login fresco. |
| 6 | Segunda sesión | Aprobada por matriz central | Se registró independientemente el resultado del segundo dispositivo. |
| 7 | Barrera SITAA | Aprobada por matriz central | Las operaciones protegidas intentadas con ambos tokens quedaron denegadas; la clase gateway/base no se persistió. |
| 8 | Restauración | Aprobada por matriz central | Se aplicó el valor aislado `ban_duration = 'none'` y se registró el resultado real. |
| 9 | Refresh anterior restaurado | Aprobada por matriz central | Se registró que los refresh tokens anteriores no recuperaron acceso en esa ejecución. |
| 10 | Login nuevo restaurado | Aprobada por matriz central | Login fresco posterior a restauración aprobado. |
| 11 | `activated_at` | Aprobada por matriz central | Conservó el valor original. |
| 12 | Historia | Aprobada por matriz central | Asignaciones e historia operativa permanecieron sin cambios. |
| 13 | Fallo Auth inyectado | Aprobada por matriz failure/recovery v11 | El reintento completó la misma operación sin segundo evento de ciclo ni repetición Auth en replays completados. |
...
| 20 | Sanitización | Aprobada por smoke test productivo | La interfaz no expuso respuestas crudas ni controles indebidos; la suspensión, denegación, restauración, sesión nueva y preservación de datos se observaron en el recorrido controlado. |
...
## 6. Smoke test de producción aprobado

El smoke test productivo controlado se ejecutó sobre el commit desplegado `18e40db3254d6c3b73b24dcd4d29ee229498b0e5`, con una autoridad B.1 exacta y una cuenta institucional estudiantil ficticia. Fue la única ejecución controlada autorizada antes del cierre, preservó la auditoría append-only y no equivale a habilitar la función para uso operativo general.

El recorrido aprobó:

- contexto y presentación de ciclo de vida sanitizados;
- suspensión coordinada del perfil y Auth;
- denegación de la sesión ya emitida y de un login fresco durante la suspensión;
- restauración Auth antes de la activación final del perfil;
- nuevo login exitoso y retorno a `/activities` después de restaurar;
- ausencia de controles administrativos para el usuario ordinario;
- preservación de asignación, actividades, asistencia, identidad e historia;
- cero respuestas crudas, errores técnicos, UUID u operation ID visibles.

| Agregado final | Resultado |
| --- | ---: |
| Operaciones B.3a | 2 |
| Desactivaciones / reactivaciones | 1 / 1 |
| Operaciones `succeeded/completed` | 2 |
| Operaciones no finales / no exitosas | 0 / 0 |
| Intentos totales / códigos de error estables | 2 / 0 |
| Referencias de auditoría de perfil / Auth | 2 / 2 |
