...
El primer intento de migración falló de forma segura mientras PostgreSQL compilaba el `DO $preflight$` inicial por un `EXISTS` exterior sin cerrar. Sólo se alcanzaron `BEGIN` y los dos `SET LOCAL`; ningún `CREATE FUNCTION`, `REVOKE`, `GRANT`, guard post-DDL o `COMMIT` fue alcanzado, no se creó objeto 0009 alguno y no fue necesario ejecutar el rollback. La migración corregida permanece local y su reejecución está pendiente. 0009 sigue no aplicada, B.2b no está cerrada y B.3/Fase C continúan pendientes.
