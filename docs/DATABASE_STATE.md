...
El primer preflight remoto de 0008 fue de sólo lectura y revirtió correctamente, pero informó `registration_trigger_drift = 1` por un falso positivo en nombres locales. El contrato corregido reconoce y valida exactamente `on_sitaa_auth_user_created` y `on_sitaa_auth_user_email_changed` sobre `auth.users`; no propone modificarlos. La reejecución del preflight corregido está pendiente y 0008 continúa sin aplicar.
