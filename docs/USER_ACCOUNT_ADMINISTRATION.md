...
La matriz de concurrencia y límites v8 aprobó los casos 17–18 y el comportamiento multisesión del camino coordinado 0010. El mismo `request_id` con payload normalizado fue idempotente; un payload distinto fue rechazado y solicitudes diferentes contra el mismo objetivo quedaron serializadas. Se observó espera real por advisory lock, captura de tiempos después del lock, `updated_at` monotónico y orden correcto de la operación más reciente.

Un lease fresco no fue reclamado prematuramente y pudo recuperarse después de cinco minutos. El intento obsoleto fue cercado y el vigente aceptado. Si el actor perdió autoridad mientras esperaba, claim, record y replay final fallaron cerrados antes de mutar o devolver estado. Otra autoridad B.1 exacta recuperó la operación sincronizada; Auth se llamó exactamente una vez y no se repitió durante la recuperación. Solicitante y finalizador permanecieron diferenciados.
...
La reactivación exige identidad coherente, Auth confirmado y, para cuentas institucionales, un programa existente y activo bloqueado con `FOR SHARE`. El orden es: advisory de ciclo, `role_assignments` en `SHARE`, Auth objetivo, perfiles ordenados por UUID, segunda autorización, programa institucional, validación, actualización y auditoría. La matriz Hosted Auth v8 observó en un entorno desechable la serialización, idempotencia, locks, leases, pérdida posterior de autoridad y recuperación del camino coordinado 0010. La desactivación aplica la barrera operativa existente sin borrar historia ni afirmar revocación física de sesiones. Al reactivar, sólo recuperan efecto asignaciones todavía activas, vigentes, válidas y compatibles.
...
En el contrato aplicado 0010, prepare, claim, record y finalize vuelven a validar B.1 después de serializar con locks. Claim no entrega replay ni lease a quien perdió autoridad; record no acepta el intento ni escribe auditoría; finalize no devuelve siquiera un replay final. Otra autoridad B.1 exacta puede recuperar la operación. La respuesta Edge es discriminada y la acción sólo redirige cuando `deactivate/account_deactivated` o `reactivate/account_reactivated` llegan como `completed` con UUID.
