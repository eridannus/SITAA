...
El primer preflight remoto de 0009 fue de sólo lectura, devolvió sus 26 categorías y terminó con `ROLLBACK`; no fue aprobado por cuatro falsos positivos del arnés. Un diagnóstico de sólo lectura confirmó el estado post-0008 y también terminó con `ROLLBACK`. La comparación local corregida usa las representaciones y ACL canónicas del snapshot, pero aún no se ha reejecutado. Por ello B.2b continúa preparada, no aplicada ni cerrada; B.3 y Fase C permanecen pendientes.
