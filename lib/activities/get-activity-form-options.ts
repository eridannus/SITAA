...
import type {
  ...
  AttentionCategory, CatalogRow, LocationType, ServiceType,
} from "@/types/catalogs";
...
function activeCatalog<T extends CatalogRow>(rows: T[]) {
  return rows.filter((row) => row.is_active !== false).sort((left, right) => {
...
export async function getActivityFormOptions(): Promise<ActivityFormOptions> {
  ...
  if (results.some((result) => result.error)) throw new Error("No fue posible cargar las opciones de actividad.");
  return {
    academicPeriods: activeCatalog(results[0].data as AcademicPeriod[]),
    programs: (results[1].data as AcademicProgram[]).filter((item) => item.is_active !== false),
    divisions: (results[2].data as Division[]).filter((item) => item.is_active !== false),
    activityTypes: activeCatalog(results[3].data as ActivityType[]),
    serviceTypes: activeCatalog(results[4].data as ServiceType[]),
    attentionCategories: activeCatalog(results[5].data as AttentionCategory[]),
    modalities: activeCatalog(results[6].data as ActivityModality[]),
    statuses: activeCatalog(results[7].data as ActivityStatus[]),
    locationTypes: activeCatalog(results[8].data as LocationType[]),
  };
}
