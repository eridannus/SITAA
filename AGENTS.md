...
La aplicación y las migraciones `0001`–`0006` ya existen, están aplicadas, verificadas y reconciliadas. `0006_structured_person_names.sql` mantiene `full_name` como compatibilidad derivada y formaliza los componentes estructurados. `0007_admin_account_directory_audit.sql` está aplicada con `COMMIT`, su preflight fue aprobado y la aplicación compatible está publicada; la migración es inmutable. La primera ejecución del verificador falló antes de crear fixtures por un defecto del arnés ya corregido localmente. Siguen pendientes su reejecución, los smoke tests y la reconciliación post-0007; el snapshot vigente continúa siendo post-0006. No crear 0008 para corregir el verificador ni describir 0007 como reconciliada hasta cerrar esas etapas. 0008 será el siguiente número disponible después de cerrar 0007, salvo que antes aparezca una corrección real de esquema que requiera una migración nueva. No modificar migraciones aplicadas ni conectarse a Supabase sin autorización expresa.
