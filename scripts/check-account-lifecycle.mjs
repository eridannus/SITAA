...
const lifecyclePageSource = fs.readFileSync(path.join(root, "app", "admin", "accounts", "[id]", "lifecycle", "page.tsx"), "utf8");
...
assert.match(dataSource, /resultRows\.length > 1/);
assert.match(dataSource, /resultRows\.length !== 1/);
assert.match(dataSource, /LIFECYCLE_DENIAL_CODES/);
assert.match(dataSource, /function isUuid/);
assert.match(dataSource, /function isTimestamp/);
assert.match(dataSource, /const LIFECYCLE_DENIAL_CODES = new Set/);
assert.match(dataSource, /Number\.isSafeInteger/);
assert.match(dataSource, /resultRows\.length > 1/);
assert.match(dataSource, /resultRows\.length !== 1/);
assert.match(dataSource, /value\.target_profile_id !== input\.targetProfileId/);
assert.match(dataSource, /!isUuid\(value\.audit_event_id\)/);
assert.match(lifecyclePageSource, /requested !== "deactivate" && requested !== "reactivate"/);
assert.doesNotMatch(lifecyclePageSource, /requested === "reactivate"[\s\S]{0,100}: "deactivate"/);
