...
`DOC-00` y `AUTH-01` están completados. Google OAuth público usa audiencia **External**, está **In production**, muestra la marca verificada y opera sobre la propiedad verificada de `sitaa.net`; solicita sólo `openid`, `userinfo.email` y `userinfo.profile`, con cero scopes sensibles o restringidos. `/acerca-de` y `/privacidad` están desplegadas, y el cierre sanitizado consta en `docs/AUTH_01_PUBLIC_OAUTH_EVIDENCE.md`. `SEM-01` es el siguiente paquete estructural activo: su contrato de producto está aprobado y documentado en [`docs/SEM_01_DESIGN.md`](docs/SEM_01_DESIGN.md), pero la implementación no ha comenzado y `0011` no existe ni está asignada. El lanzamiento inicial no requiere una página de términos.
