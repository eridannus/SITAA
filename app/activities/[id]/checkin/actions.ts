...
function actionStatus(action: "open" | "close" | "regenerate", error: RpcError) {
  ...
  if (/todavia.*no.*puede.*abrirse|aun.*no.*puede.*abrirse|not.*yet|too.*early|before.*open|opening.*window|ventana.*apertura|antes.*apertura/.test(raw)) return action + "-not-yet-available";
