...
export async function createActivity(
  ...
  if (Object.keys(errors).length > 0) {
    return invalidState(values, errors);
  }
  ...
  if (profileError || !profile) {
    return invalidState(values, {}, "Tu cuenta necesita un perfil institucional activo.");
  }
  ...
  if (periodError || !periods || periods.length !== 1) {
    return invalidState(
      ...
      "No es posible crear actividades hasta configurar un periodo académico activo.",
    );
  }
